...
document.body.onload = async () => {
  const pokemon_name = select.options[select.selectedIndex].value + ".json";
...
function loadPokemonInfo(pokemon) {
  ...
  for (let i = 0; i < pokemon.types.length; i++) {
    ul.innerHTML += `<li class="list-group-item"> Ability : ${pokemon.abilities[i].ability.name}</li>`;
  }
}
...
function loadPokemonMoves(pokemon) {
  ...
  for (let i = 0; i < pokemon.moves.length; i++) {
    let level_learned =
      pokemon.moves[i].version_group_details.pop().level_learned_at;
    if (level_learned == 0) continue;
...
function loadChartCSS(pokemon) {
  ...
  for (let i = 0; i < pokemon.stats.length; i++) {
    chart_css.innerHTML += `<tr scope="row">
			...
			<td style="--size: calc(${pokemon.stats[i].base_stat}/200)">${pokemon.stats[i].base_stat}</td>
		</tr>`;
  }
}
